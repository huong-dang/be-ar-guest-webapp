...
// Using a grid, each row can take up a max grid width of 12. So, when the view is in mobile (xs) each grid (image
// card) takes up the full width 12. In desktop mode which is denoted as "md" and up I've set it to a value of 6 -
// meaning half the row (which renders two cards per row). I can try to explain this better later on  cause its a
// little confusing.

// Also, I removed the marginTop: 30% on each card image. Instead, I spaced them out by setting a spacing property on
// the outermost Grid component. It's set to 24,  but you can change that as you need. And I increased the width of
// parkImage from 40% to 80%. Again, not necessary, but it looked weird at 40%.
