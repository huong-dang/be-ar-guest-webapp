...
router.post('/add', async (req, res) => {
    try {
        const {restaurantID, itemName, itemDescription, secret, vegan, substitution, itemStatus, x, z} = req.body;

        // Non required fields are x, z, itemDescription, and substitution
        if (_.isNil(restaurantID) || _.isNil(itemName) || _.isNil(vegan) || _.isNil(secret) || _.isNil(itemStatus)) {
            throw new Error('Missing restaurantID, itemName, secret, itemStatus, or vegan.');
...
async function itemDoesExist(itemName, restaurantID) {
    const query  = `select itemID from Item where itemName = ${sqlstring.escape(itemName)} and restaurantID = ${sqlstring.escape(restaurantID)};`;
    const result = await DB.runQuery(query);
